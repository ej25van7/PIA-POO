...
int startGameEngine(void *ptrMsg){
    ...
    Model* model = new Model("models/Bola/bola.fbx", translate, camera);
    ...
    model->setTranslate(&translate);
    try {
        std::vector<Animation> animations = Animation::loadAllAnimations("models/Bola/bola.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
